-- Sửa lỗi logic tự động điền khuyết form_code để hỗ trợ các mã có thêm ký tự phụ (như CS0001A, CS0001B)
  ...
  if tg_table_name = 'forms' then
    if new.form_code is null then
      if new.form_type = 'CAU_AN' then
        -- Lấy ra tất cả các số đã tồn tại (bao gồm cả mã có chữ cái phụ)
