-- Sửa lỗi 'relation does not exist' do gọi lại CTE ngoài phạm vi và tối ưu hóa thuật toán
    ...
    if new.form_code is null then
      ...
      if new.form_type = 'CAU_AN' then
        WITH existing_numbers AS (
            ...
            UNION ALL
            SELECT 0 -- Chèn số 0 ảo để thuật toán tự động tìm được khoảng trống số 1 nếu số 1 bị thiếu
        )
